...
# set up volume 
NOW="$(date +%s)"
VOLUME_NAME="write.test.$NOW"
UG_NAME="write.test.ug.$NOW"
RG_NAME="write.test.rg.$NOW"

printf "Y\n" | syndicate setup "$ADMIN" "$PRIVKEY" "$MS" -c "$CONFDIR/syndicate.conf"

syndicate -c "$CONFDIR/syndicate.conf" create_volume email="$ADMIN" name="$VOLUME_NAME" description="test writes" blocksize="$BLOCKSIZE" private=False allow_anon=True
